...
class TestBettingService:
    ...
    async def test_create_bet_success(
        self, betting_service, test_user, mock_wallet_service
    ):
    ...
    async def test_accept_bet_success(
        self, betting_service, test_bet, test_user_2, mock_wallet_service
    ):
        """Test successful bet acceptance"""
        with patch.object(betting_service, "wallet_service", mock_wallet_service):
            bet = await betting_service.accept_bet(
                bet_id=test_bet.id, acceptor_id=test_user_2.id
            )
    ...
    async def test_accept_bet_not_found(self, betting_service, test_user_2):
        ...
        with pytest.raises(ValidationError, match="Bet not found"):
            await betting_service.accept_bet(bet_id=99999, acceptor_id=test_user_2.id)

    @pytest.mark.asyncio
    async def test_accept_bet_wrong_status(
        self, betting_service, test_bet, test_user_2
    ):
    ...
    async def test_resolve_bet_success(
        self, betting_service, test_bet, test_user, mock_wallet_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_resolve_bet_already_resolved(
        self, betting_service, test_bet, test_user
    ):
    ...
    async def test_cancel_bet_success(self, betting_service, test_bet, test_user):
        ...
        bet = await betting_service.cancel_bet(
            bet_id=test_bet.id, user_id=test_user.id, reason="Test cancellation"
        )
    ...
    async def test_cancel_bet_wrong_user(self, betting_service, test_bet, test_user_2):
        ...
        with pytest.raises(ValidationError, match="Only bet creator can cancel"):
            await betting_service.cancel_bet(
                bet_id=test_bet.id, user_id=test_user_2.id, reason="Test cancellation"
            )
    ...
    async def test_get_user_bets_with_status_filter(
        self, betting_service, test_user, test_bet
    ):
        """Test getting user's bets with status filter"""
        bets = await betting_service.get_user_bets(
            user_id=test_user.id, status=BetStatus.PENDING, limit=10, offset=0
        )
    ...
    @pytest.mark.asyncio
    async def test_bet_locking_race_condition(
        self, betting_service, test_bet, test_user_2
    ):
    ...
    @pytest.mark.asyncio
    async def test_bet_expiration_handling(
        self, betting_service, test_bet, test_user_2
    ):
    ...
    @pytest.mark.asyncio
    async def test_bet_limit_enforcement(
        self, betting_service, test_user, test_bet_limit
    ):
        ...
        with pytest.raises(
            ValidationError, match="Bet would exceed daily betting limit"
        ):
            await betting_service.create_bet(
                creator_id=test_user.id,
                title="Limit Test",
    ...
    async def test_fund_distribution_winner_a(self, betting_service, test_bet):
        ...
        with patch.object(betting_service, "wallet_service", mock_wallet_service):
            ...
            assert (
                mock_wallet_service.unlock_funds.call_count == 2
            )  # Unlock both stakes
            assert (
                mock_wallet_service.add_funds.call_count == 1
    ...
    async def test_audit_logging(self, betting_service, test_bet, test_user):
        ...
        with patch.object(betting_service, "_audit_log") as mock_audit:
            await betting_service.cancel_bet(
                bet_id=test_bet.id, user_id=test_user.id, reason="Test audit"
            )
    ...
    @pytest.mark.asyncio
    async def test_bet_version_optimistic_locking(
        self, betting_service, test_bet, test_user
    ):
        ...
        await betting_service.cancel_bet(
            bet_id=test_bet.id, user_id=test_user.id, reason="Version test"
        )
    ...
    async def test_concurrent_bet_operations(
        ...
        results = await asyncio.gather(
            operation_1(), operation_2(), return_exceptions=True
        )
