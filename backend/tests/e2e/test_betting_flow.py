...
class TestBettingFlow:
    ...
    @pytest.mark.asyncio
    async def test_complete_betting_workflow(self, async_test_client, test_user_token, test_user_2_token):
        ...
        # Verify participants
        creator_participant = next(p for p in final_bet["participants"] if p["role"] == "creator")
        acceptor_participant = next(p for p in final_bet["participants"] if p["role"] == "acceptor")
    ...
    @pytest.mark.asyncio
    async def test_bet_cancellation_workflow(self, async_test_client, test_user_token, test_user_2_token):
    ...
    @pytest.mark.asyncio
    async def test_bet_expiration_workflow(self, async_test_client, test_user_token, test_user_2_token):
        ...
        await async_test_client.db.execute(
            update(Bet)
            .where(Bet.id == bet_id)
            .values(expires_at=async_test_client.db.execute("SELECT NOW() - INTERVAL '1 hour'").scalar())
        )
    ...
    @pytest.mark.asyncio
    async def test_multi_user_betting_scenario(self, async_test_client, test_user_token, test_user_2_token):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_bet_operations(self, async_test_client, test_user_token, test_user_2_token):
        ...
        successful_operations = [r for r in results if not isinstance(r, Exception) and r.status_code == 200]
    ...
    @pytest.mark.asyncio
    async def test_bet_statistics_tracking(self, async_test_client, test_user_token, test_user_2_token):
    ...
    @pytest.mark.asyncio
    async def test_error_recovery_scenarios(self, async_test_client, test_user_token, test_user_2_token):
    ...
    async def test_authentication_and_authorization(self, async_test_client):
        ...
        accept_response = await async_test_client.post("/api/v1/bets/accept", json={"bet_id": 1})
        assert accept_response.status_code == 401
        
        resolve_response = await async_test_client.post("/api/v1/bets/resolve", json={"bet_id": 1, "outcome": "winner_a"})
